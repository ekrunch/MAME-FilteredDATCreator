<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <!-- 2. The Exclusion Template: This rule overrides the Identity Template.
         ...
         a) Has the 'cloneof' attribute set (@cloneof)
         OR
         b) Contains a 'softwarelist' child element (softwarelist)
         OR
         c) Has the 'isdevice' attribute set to 'yes' (@isdevice = 'yes').
         
         These machines are FILTERED OUT (suppressed), ensuring ONLY machine elements 
         that are **originals**, are **not software lists**, AND are **not devices** are SELECTed (kept) in the output. -->
    <xsl:template match="machine[@cloneof or softwarelist or @isdevice = 'yes']"/>
